...
public:
  ...
  inline const T *operator[](const int i) const { return PtrList_[i]; }

  //- return element pointer reference
  T &operator[](const int i) { return *PtrList_[i]; }
...
public:
  A() : a1_(100), a2_(1e-06), jPtr_(NULL)
  {
    jPtr_ = new int[a1_];
    for (int i = 0; i < a1_; i++)
    {
  ...
  A(int a1, double a2) : a1_(a1), a2_(a2), jPtr_(NULL)
  {
    jPtr_ = new int[a1_];
    for (int i = 0; i < a1_; i++)
    {
...
template <typename T>
PtrList<T>::PtrList() : size_(50), PtrList_(NULL)
{
...
template <typename T>
PtrList<T>::PtrList(int size) : size_(size), PtrList_(NULL)
{
  PtrList_ = new T *[size_];
  for (int i = 0; i < size_; i++)
  {
...
int main()
{
  ...
  {
    cout << "At i = " << i << ", a1 is " << aa[i].a1() << ", a2 is "
         << aa[i].a2() << endl;
  }
