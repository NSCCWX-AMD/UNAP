...
{
  return (rhx2 * (u[(i - 1) * (N + 1) + j] - 2 * u[i * (N + 1) + j] +
                  u[(i + 1) * (N + 1) + j]) +
          rhy2 * (u[i * (N + 1) + j - 1] - 2 * u[i * (N + 1) + j] +
                  u[i * (N + 1) + j + 1]));
}
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < N; i++)
    ...
    {
      b[(i - 1) * (N - 1) + j - 1] =
          bAll[i * (N + 1) + j] - Au(xAll, rhx2, rhy2, i, j);
    }
  ...
  for (int i = 1; i < N; i++)
    for (int j = 1; j < N; j++)
    {
      ...
      nZerosCells[(i - 1) * (N - 1) + j - 1 + 1] =
          n1 - n0 + nZerosCells[(i - 1) * (N - 1) + j - 1];
    }
  ...
  forAll(i, local_offdiag_size) { local_offdiag_row[i] -= ilower; }

  scalarVector *patchCoeffsPtr =
      new scalarVector(local_offdiag_data, local_offdiag_size);
  labelVector *faceCellsPtr =
      new labelVector(local_offdiag_row, local_offdiag_size);
  ...
  lduMatrix &lduA = coo2ldu(local_diag_data,
                            local_diag_row,
                            local_diag_col,
                            local_size,
                            local_diag_size,
