...
void UNAP::MGSolver::initVcycle(PtrList<scalarVector> &coarseCorrFields,
                                PtrList<scalarVector> &coarseSources) const
{
  forAll(leveli, agglomeration_.size())
  {
    ...
    scalarVector *coarseCorrFieldsLeveliPtr =
        new scalarVector(nCells, this->commcator_);
    scalarVector *coarseSourcesLeveliPtr =
        new scalarVector(nCells, this->commcator_);
...
void UNAP::MGSolver::Vcycle(scalarVector &psi,
                            ...
                            PtrList<scalarVector> &coarseSources) const
{
  ...
  for (label leveli = 0; leveli < coarsestLevel; leveli++)
  {
    ...
    if (nPreSweeps_)
    {
      ...
      smoothers_[leveli + 1].smooth(coarseCorrFields[leveli],
                                    agglomeration_.coarseMatrixLevels(leveli),
                                    coarseSources[leveli],
                                    nPreSweeps_ + leveli);

      scalarVector ACf(nCells, this->commcator_);

      //- scale coarse-grid correction field
      //  but not on the coarsest level because it evaluates to 1
      if (scaleCorrection_ && leveli < coarsestLevel - 1)
      {
        scalar sf = scalingFactor(ACf,
        ...
        IFNOT_SWACC
        {
          forAll(cellI, nCells) { coarseCorrFieldsPtr[cellI] *= sf; }
        }
#ifdef SW_SLAVE
        else
        {
      ...
      agglomeration_.coarseMatrixLevels(leveli).spMV(ACf,
                                                     coarseCorrFields[leveli]);
      ...
      IFNOT_SWACC
      {
        forAll(cellI, nCells) { coarseSourcesPtr[cellI] -= ACfPtr[cellI]; }
      }
#ifdef SW_SLAVE
      else
      {
    ...
    agglomeration_.restrictField(
        coarseSources[leveli + 1], coarseSources[leveli], leveli + 1);
  }
  ...
  if (coarsestLevel >= 0)
  {
    solveCoarsestLevel(coarseCorrFields[coarsestLevel],
  ...
  for (label leveli = coarsestLevel - 1; leveli >= 0; leveli--)
  {
    ...
    scalarVector preSmoothedCoarseCorrField(nCells, this->commcator_);
    ...
    agglomeration_.prolongField(
        coarseCorrFields[leveli], coarseCorrFields[leveli + 1], leveli + 1);
  ...
  if (coarsestLevel >= 0)
  {
    agglomeration_.prolongField(finestCorrection, coarseCorrFields[0], 0);
  }
  ...
  if (scaleCorrection_)
  {
    ...
    scalar fsf =
        scalingFactor(Apsi, finestCorrection, finestResidual, finestMatrix_);
    ...
    IFNOT_SWACC
    {
      forAll(i, nCells) { psiPtr[i] += fsf * finestCorrectionPtr[i]; }
    }
#ifdef SW_SLAVE
    else
    {
  ...
  else
  {
    ...
    IFNOT_SWACC
    {
      forAll(i, nCells) { psiPtr[i] += finestCorrectionPtr[i]; }
    }
#ifdef SW_SLAVE
    else
    {
  ...
  smoothers_[0].smooth(psi, finestMatrix_, source, nFinestSweeps_);
}
