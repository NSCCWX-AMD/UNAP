...
) const
{
	...
	const matrix& coarsestMatrix = agglomeration_.coarseMatrixLevels(coarsestLevel);
	...
	if(coarsestMatrix.symm())
	{
		...
		coarseSolverPerf = PCGSolver.solve(coarsestCorrField, coarsestMatrix, coarsestSource);
	}
	else
	{
		...
		coarseSolverPerf = PBiCGStabSolver.solve(coarsestCorrField, coarsestMatrix, coarsestSource);
	}
}
