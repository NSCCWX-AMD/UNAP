...
void UNAP::chebySmoother::smooth(scalarVector &x,
                                 const lduMatrix &A,
                                 ...
                                 const label nSweeps) const
{
  ...
  {
    eigenDiagPCG calEigen(A, x, b, precond, nDiagPCGs_);
  ...
  else
  {
    ...
    for (label sweep = 1; sweep < nSweeps; sweep++)
    {
      ...
      IFNOT_SWACC
      {
        forAll(cell, nCells)
        {
          wPtr[cell] = dtemp1 * wPtr[cell] +
                       dtemp2 * rDPtr[cell] * (bPtr[cell] - pPtr[cell]);
          xPtr[cell] += wPtr[cell];
        }
      }
#ifdef SW_SLAVE
