...
void swTimer::startTimer(string in) { timeStart[in] = getSystemTime(); }
...
void swTimer::printTimer(string in)
{
  ...
  else
  {
    cout << it->first << " Time: " << it->second
         << "s, count: " << count[it->first]
         << ",  average: " << it->second / count[it->first] << "s" << endl;
  }
}
...
//- interfaces for c
void swTimerStart(const char *name) { swTimer::startTimer(name); }

void swTimerEnd(const char *name) { swTimer::endTimer(name); }

void swTimerPrintAll() { swTimer::printTimer(); }

void swTimerPrint(const char *name) { swTimer::printTimer(name); }

void swTimerMaxSum() { swTimer::maxTimeSum(); }

//- interfaces for Fortran
void swtimerstart_(const char *name) { swTimer::startTimer(name); }

void swtimerend_(const char *name) { swTimer::endTimer(name); }

void swtimerprintall_() { swTimer::printTimer(); }

void swtimerprint_(const char *name) { swTimer::printTimer(name); }

void swtimermaxsum_() { swTimer::maxTimeSum(); }
