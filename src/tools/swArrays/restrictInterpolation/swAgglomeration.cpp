...
void UNAP::swRestInterMap::initRestInterSize()
{
  ...
  if (!restFirstUse_)
  {
    ...
    forAll(i, levelSize) { restFirstUse_[i] = true; }
  }
  ...
  if (!faceRestFirstUse_)
  {
    ...
    forAll(i, levelSize) { faceRestFirstUse_[i] = true; }
  }
  ...
  if (!interFirstUse_)
  {
    ...
    forAll(i, levelSize) { interFirstUse_[i] = true; }
  }
  ...
  if (!aggMatrixUpperFirstUse_)
  {
    ...
    forAll(i, levelSize) { aggMatrixUpperFirstUse_[i] = true; }
  }
}

void UNAP::swRestInterMap::initRestStruct(Vector<scalar> &cf,
                                          ...
                                          const label fineLevelIndex)
{
  ...
  if (restFirstUse_[fineLevelIndex])
  {
    ...
    forAll(i, slaveCores) { range[i] = new swInt[4]; }
...
void UNAP::swRestInterMap::initInterStruct(Vector<scalar> &ff,
                                           ...
                                           const label levelIndex)
{
  ...
  if (interFirstUse_[levelIndex])
  {
    ...
    forAll(i, cSize)
    {
      ...
      {
        range[fMaxSlavePos][2] =
            range[fMaxSlavePos][2] < i ? range[fMaxSlavePos][2] : i;
      }
      else if (i > range[fMaxSlavePos][3])
      {
        range[fMaxSlavePos][3] =
            range[fMaxSlavePos][3] > i ? range[fMaxSlavePos][3] : i;
      }

      if (i < range[fMinSlavePos][2])
      {
        range[fMinSlavePos][2] =
            range[fMinSlavePos][2] < i ? range[fMinSlavePos][2] : i;
      }
      else if (i > range[fMinSlavePos][3])
      {
        range[fMinSlavePos][3] =
            range[fMinSlavePos][3] > i ? range[fMinSlavePos][3] : i;
      }
    }
...
{
template <>
void matrix::agglomeration::restrictField(Vector<scalar> &cf,
                                          const Vector<scalar> &ff,
...
template <>
void matrix::agglomeration::prolongField(Vector<scalar> &ff,
                                         const Vector<scalar> &cf,
