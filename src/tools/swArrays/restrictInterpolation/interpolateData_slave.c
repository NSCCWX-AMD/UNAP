...
void interpolateData_slave(interStruct *is)
{
  ...
  const swFloat *c_hostPtr = is_slave.cPtr;
  ...
  for (cycleI = 0; cycleI < slaveCycles; ++cycleI)
  {
    ...
    for (i = sizePerCycle; i <= cLenLocal; i += sizePerCycle)
    {
      ...
      get_reply = 0;
      athread_get(PE_MODE,
                  ((swFloat *)c_hostPtr) + offset_vector,
                  c_slavePtr,
      ...
      volatile swInt interMapSize =
          offsetMap_slavePtr[sizePerCycle] - offsetMap_slavePtr[0];
      get_reply = 0;
      athread_get(PE_MODE,
      ...
      {
        volatile swInt locSize =
            offsetMap_slavePtr[j + 1] - offsetMap_slavePtr[j];
        swInt k = 0;

        for (k = 0; k < locSize; ++k)
        {
          volatile swInt fPos =
              map_slavePtr[offsetMap_slavePtr[j] + k - offsetMap_slavePtr[0]];
      ...
      sizePerCycle = ((remaining < sizePerCycle) && (remaining > 0))
                         ? remaining
                         : sizePerCycle;
    }
