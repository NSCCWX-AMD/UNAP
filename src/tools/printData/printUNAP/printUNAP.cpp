...
void UNAP::printLDUMatrix(const lduMatrix &A, const char *fileName)
{
  ...
  forAll(i, nCells)
  {
    fout << i << " " << i << " " << setiosflags(ios::scientific)
         << setprecision(output_precision) << diagPtr[i] << std::endl;
  }

  //- upper
  forAll(i, nFaces)
  {
    fout << lPtr[i] << " " << uPtr[i] << " " << setiosflags(ios::scientific)
         << setprecision(output_precision) << upperPtr[i] << " ";

    //- if there is lower
    if (!symm)
    {
      fout << setiosflags(ios::scientific) << setprecision(output_precision)
           << lowerPtr[i];
    }
