...
UNAP::lduDILUPrecond::lduDILUPrecond(const lduMatrix &A) : rD_(A.diag())
{
  APtr_ = &A;
  calcReciprocalD(rD_, A);
}

void UNAP::lduDILUPrecond::calcReciprocalD(scalarVector &rD, const lduMatrix &A)
{
  ...
  forAll(i, nCells) { rDPtr[i] = 1.0 / rDPtr[i]; }
}

void UNAP::lduDILUPrecond::precondition(scalarVector &w,
                                        const scalarVector &r) const
{
  ...
  forAll(i, nCells) { wPtr[i] = rDPtr[i] * rPtr[i]; }
