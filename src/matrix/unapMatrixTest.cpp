...
bool UNAP::matrix::solverPerformance::checkConvergence(const scalar Tolerance,
                                                       const label iter,
                                                       const label minIter)
{
...
bool UNAP::matrix::solverPerformance::checkConvergence(
    const scalar Tolerance, const scalar RelTolerance)
{
  if ((finalResidual_ < Tolerance ||
       (RelTolerance > SMALL &&
        finalResidual_ <= RelTolerance * initialResidual_)))
...
bool UNAP::matrix::solverPerformance::checkSingularity(const scalar residual)
{
  if (residual > SMALL)
  {
    singular_ = false;
