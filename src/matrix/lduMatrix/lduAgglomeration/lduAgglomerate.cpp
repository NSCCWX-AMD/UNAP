...
void UNAP::lduAgglomeration::agglomerate(const scalarVector &faceWeights)
{
  ...
  while (nCreatedLevels < maxLevels_ - 1)
  {
    ...
    labelVector *finalAgglomPtr = &agglomerate(
        nCoarseCells, matrixLevel(nCreatedLevels), *faceWeightsPtr);
    ...
    const scalarVector &faceWeights)
{
  ...
  //- memory management
  {
    ...
    forAll(facei, upperAddr.size()) { nNbrs[upperAddr[facei]]++; }

    forAll(facei, lowerAddr.size()) { nNbrs[lowerAddr[facei]]++; }
    ...
    {
      cellFaces[cellFaceOffsets[upperAddr[facei]] + nNbrs[upperAddr[facei]]] =
          facei;
    ...
    {
      cellFaces[cellFaceOffsets[lowerAddr[facei]] + nNbrs[lowerAddr[facei]]] =
          facei;
  ...
  for (label celli = 0; celli < nFineCells; celli++)
  {
    ...
    if (coarseCellMap[celli] < 0)
    {
      ...
      for (label faceOs = cellFaceOffsets[celli];
      ...
      else
      {
        ...
        if (clusterMatchFaceNo >= 0)
        {
          ...
          coarseCellMap[celli] =
              MAX(coarseCellMap[upperAddr[clusterMatchFaceNo]],
