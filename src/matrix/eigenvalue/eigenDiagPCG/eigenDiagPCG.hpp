...
private:
  ...
  void determineEigRange(scalar **TriMatrix,  /// tridiagonal matrix
  ...
  void computeMaxEig(
      ...
      const label nPCGs,           /// nDiagPCGs_
      const label
          k  /// kth largest eigenvalue, default k=1 for the maximum eigenvalue
  ) const;

  //- diagPCG loops
  void diagPCGLoops(const matrix &A,
                    scalarVector &x,
